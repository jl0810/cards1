...
import { X, Search, CreditCard, Check, Sparkles, TrendingUp, ChevronRight, ChevronLeft, Link2 } from "lucide-react";
...
function calculateMatchScore(
    ...
    } else if (normalizedInstitution && normalizedInstitution.includes(normalizedIssuer)) {
        score += 45;
        reasons.push(`Issued by ${product.issuer}`);
    } else if (normalizedInstitution && normalizedIssuer.includes(normalizedInstitution)) {
        score += 45;
        reasons.push(`Issued by ${product.issuer}`);
    }
    ...
    const specialKeywords = ["platinum", "preferred", "reserve", "sapphire", "premier", "freedom", "venture", "executive", "signature", "world elite", "elite", "aadvantage", "skypass"];
...
function ConfidenceBadge({ score }: { score: number }) {
    ...
    return (
        <div className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${color}`}>
...
export function CardProductMatcher({
    ...
    const scoredProducts = useMemo<ScoredProduct[]>(() => {
        return products.map(product => {
            const { score, reasons } = calculateMatchScore(product, accountName, institutionName);
            return {
                ...product,
                matchScore: score,
    ...
    const goodMatches = filteredProducts.filter(p => p.matchScore >= 70 && p.matchScore < 85);
    const possibleMatches = filteredProducts.filter(p => p.matchScore >= 50 && p.matchScore < 70);
    ...
    const linkProduct = async (productId: string | null) => {
        ...
        toast.promise(linkPromise, {
            ...
            success: () => {
                onSuccess();
                return productId ? `Linked ${productName} successfully!` : 'Card unlinked';
            },
            error: productId ? 'Failed to link card product' : 'Failed to unlink card'
        });
            ...
            <div className="flex flex-col h-full p-4 space-y-4 min-w-[80px]">
                ...
                <div className="flex-1 flex flex-col items-center justify-center space-y-6">
                    ...
                    <div className="text-center">
                        ...
                        <p className="text-2xl font-bold text-emerald-400">{stats?.linkedAccounts || 0}</p>
                        <p className="text-[10px] text-slate-400 uppercase tracking-wider">Linked</p>
                    </div>
                    ...
                    <div className="text-center">
                        ...
                        <p className="text-2xl font-bold text-slate-400">{stats?.unlinkAccounts || 0}</p>
                        <p className="text-[10px] text-slate-400 uppercase tracking-wider">Unlinked</p>
                    </div>
                </div>
                            ...
                            {isExpanded ? (
                                ...
                                >
                                    ...
                                    <div className="flex items-center justify-between p-6 border-b border-white/5 flex-shrink-0">
                                        <div className="flex-1 min-w-0">
                                            <h2 className="text-xl font-bold text-white truncate">Link Card Product</h2>
                                            <p className="text-sm text-slate-400 mt-1 truncate">{accountName}</p>
                                        </div>
                                        <div className="flex items-center gap-2 flex-shrink-0">
                                            <button
                                    ...
                                    <div className="flex-1 overflow-y-auto p-4 space-y-4">
                                        ...
                                        {loading ? (
                                            ...
                                            <div className="space-y-4">
                                                ...
                                                {(searchQuery || (bestMatches.length === 0 && goodMatches.length === 0)) && otherMatches.length > 0 && (
                                                    <div className="space-y-2">
