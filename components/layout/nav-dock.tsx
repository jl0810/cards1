    ...
    <nav className="fixed bottom-6 left-1/2 transform -translate-x-1/2 bg-glass-nav backdrop-blur-2xl rounded-full px-2 py-2 flex items-center gap-1 shadow-2xl z-50">
      {tabs.map((tab) => (
        <button
          ...
          className={`relative px-4 py-3 rounded-full transition-all duration-300 flex items-center justify-center min-w-[44px] min-h-[44px] ${
            activeTab === tab.id
              ? "text-white"
        ...
        >
          ...
          <div className="relative z-10 flex flex-col items-center gap-1">
            <tab.icon
              className={`w-6 h-6 ${activeTab === tab.id ? "text-white" : ""} ${
                tab.id === "admin" ? "text-purple-400" : ""
              }`}
