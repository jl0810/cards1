...
export class SecurityTester {
  ...
  static generateReport(results: {
    ...
    pathTraversal: ReturnType<typeof SecurityTester.testPathTraversalValidation>;
  }): string {
    const report = [
      ...
      results.xss.failed.length > 0 ? `Failed payloads: ${results.xss.failed.length}` : '',
      '',
      '## SQL Injection Protection', 
      results.sql.passed ? '✅ PASSED' : '❌ FAILED',
      results.sql.failed.length > 0 ? `Failed payloads: ${results.sql.failed.length}` : '',
      '',
      '## Path Traversal Protection',
      results.pathTraversal.passed ? '✅ PASSED' : '❌ FAILED',
      results.pathTraversal.failed.length > 0 ? `Failed payloads: ${results.pathTraversal.failed.length}` : '',
      '',
