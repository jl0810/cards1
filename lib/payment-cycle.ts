...
export function calculatePaymentCycleStatus(
  ...
  // Uncomment for debugging payment cycle logic
...
export function sortAccountsByPaymentPriority(
  ...
  const priorityDiff =
    getPaymentCyclePriority(a.paymentCycleStatus) -
    getPaymentCyclePriority(b.paymentCycleStatus);
  if (priorityDiff !== 0) return priorityDiff;

  // Secondary sort: For STATEMENT_GENERATED, sort by due date (soonest first)
  if (
    a.paymentCycleStatus === "STATEMENT_GENERATED" &&
    b.paymentCycleStatus === "STATEMENT_GENERATED"
  ) {
  ...
  if (
    a.paymentCycleStatus === "PAID_AWAITING_STATEMENT" &&
    b.paymentCycleStatus === "PAID_AWAITING_STATEMENT"
  ) {
