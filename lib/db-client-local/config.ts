 ...
 *
 * - Localhost/127.0.0.1: Disables SSL (for SSH tunnels to Hetzner)
 * - Remote with sslmode=disable: Disables SSL
 * - Remote default: Uses SSL with rejectUnauthorized: false (for Supabase/cloud)
 */
export function getConnectionConfig(url: string): ConnectionConfig {
  ...
  try {
    ...
    if (isLocal || sslDisabled) {
      console.log(
        `[db-client] Connecting to ${parsed.hostname}:${parsed.port} (SSL disabled)`,
      );
      return {
        ssl: false,
    ...
    console.log(
      `[db-client] Connecting to ${parsed.hostname}:${parsed.port} (SSL enabled)`,
    );
    return {
    ...
    console.warn(
      "[db-client] Could not parse DATABASE_URL, using SSL defaults",
    );
    return {
      ssl: { rejectUnauthorized: false },
      maxConnections: 10,
