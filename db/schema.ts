...
export const plaidAccounts = cardsSchema.table("plaid_accounts", {
    id: uuid("id").primaryKey().defaultRandom(),
    ...
    familyMaskOfficialUnique: uniqueIndex("family_mask_official_idx").on(table.familyMemberId, table.mask, table.officialName),
}));

export const accountExtended = cardsSchema.table("account_extended", {
    id: uuid("id").primaryKey().defaultRandom(),
    plaidAccountId: uuid("plaid_account_id").references(() => plaidAccounts.id, { onDelete: "cascade" }).unique().notNull(),
...
export const plaidTransactions = cardsSchema.table("plaid_transactions", {
    ...
    accountId: text("account_id").references(() => plaidAccounts.accountId, { onDelete: "cascade" }).notNull(),
...
export const transactionExtended = cardsSchema.table("transaction_extended", {
    id: uuid("id").primaryKey().defaultRandom(),
    plaidTransactionId: uuid("plaid_transaction_id").references(() => plaidTransactions.id, { onDelete: "cascade" }).unique().notNull(),
    matchedBenefitId: uuid("matched_benefit_id").references(() => cardBenefits.id),
...
export const cardProducts = cardsSchema.table("card_products", {
    id: uuid("id").primaryKey().defaultRandom(),
    ...
    issuerProductUnique: uniqueIndex("issuer_product_idx").on(table.issuer, table.productName),
}));
...
export const cardBenefits = cardsSchema.table("card_benefits", {
    id: uuid("id").primaryKey().defaultRandom(),
    cardProductId: uuid("card_product_id").references(() => cardProducts.id, { onDelete: "cascade" }).notNull(),
...
export const benefitUsage = cardsSchema.table("benefit_usage", {
    id: uuid("id").primaryKey().defaultRandom(),
    cardBenefitId: uuid("card_benefit_id").references(() => cardBenefits.id, { onDelete: "cascade" }).notNull(),
    plaidAccountId: uuid("plaid_account_id").references(() => plaidAccounts.id, { onDelete: "cascade" }).notNull(),
    ...
    benefitAccountPeriodUnique: uniqueIndex("benefit_account_period_idx").on(table.cardBenefitId, table.plaidAccountId, table.periodStart),
}));
