...
export async function POST(req: Request) {
  try {
    ...
    return Errors.internal(error instanceof Error ? error.message : 'Sync failed');
  }
}
  ...
  try {
    ...
    if (!userProfile) {
      return successResponse({
        synced: false,
        clerkId: userId,
        message: 'User not found in database. Click "Sync Now" to create profile.',
      });
    }
    ...
    return Errors.internal(error instanceof Error ? error.message : 'Failed to check status');
  }
}
