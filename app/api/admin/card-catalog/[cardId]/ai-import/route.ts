/**
 * Admin AI Import API for specific card
 * Import benefits for a specific card using AI extraction
 *
 * @module app/api/admin/card-catalog/[cardId]/ai-import
...
export async function POST(
  ...
  return withAdmin(async () => {
    ...
    logger.info("[AI Import] Starting benefits import for card", {
    ...
    try {
      ...
      logger.info("[AI Import] Found existing benefits", {
        count: existingBenefits.length,
      });

      // Extract benefits for this specific card
      logger.info("[AI Import] Extracting benefits for card", {
        productName: card.productName,
      });
      ...
      if (!response.ok) {
        ...
        logger.error("[AI Import] Gemini API error", { errorText });
        return NextResponse.json(
          {
      ...
      const responseText =
        data?.candidates?.[0]?.content?.parts?.[0]?.text || '{"benefits": []}';

      logger.debug("[AI Import] Raw Gemini response", { data });
      logger.debug("[AI Import] Extracted text", {
      ...
      try {
        ...
        logger.error("[AI Import] Failed to parse benefits", {
          responseText: responseText.substring(0, 500),
          error: e,
        });
        return NextResponse.json(
            ...
            if (existingBenefit) {
              ...
              if (oldAmount !== newAmount) {
                if (oldAmount === null && newAmount !== null) {
                  ...
                  changes.push(
                    `amount changed from $${oldAmount} to unlimited`,
                  );
                } else if (
                  oldAmount !== null &&
                  newAmount !== null &&
                  ...
                  const change =
                    newAmount > oldAmount ? "increased" : "decreased";
                  changes.push(
                    `amount ${change} from $${oldAmount} to $${newAmount}`,
                  );
                }
              }
              ...
              // Compare timing
              if (
                existingBenefit.timing !== (benefitData.timing || "Annually")
              ) {
                changes.push(
                  `timing changed from ${existingBenefit.timing} to ${benefitData.timing || "Annually"}`,
                );
              }
              ...
              if (
                ...
                (benefitData.type || "STATEMENT_CREDIT")
              ) {
                changes.push(
                  `type changed from ${existingBenefit.type} to ${benefitData.type || "STATEMENT_CREDIT"}`,
                );
              }
              ...
              const newDesc = (
                benefitData.description || benefitData.benefit
              ).trim();
              if (
                oldDesc !== newDesc &&
                Math.abs(oldDesc.length - newDesc.length) > 10
              ) {
