...
describeIf("Supabase Vault Integration", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      if (!vaultCheck[0]?.exists) {
        throw new Error(
          "Supabase Vault extension is not installed on this database",
        );
      }
  ...
  afterAll(async () => {
    if (testSecretId && prisma) {
      console.log("🧹 Cleaning up Vault secret:", testSecretId);
      try {
        // Attempt to delete the secret directly from the vault.secrets table
        // Note: This requires permissions usually available to the connection used for testing
        await prisma.$queryRaw`DELETE FROM vault.secrets WHERE id = ${testSecretId}::uuid`;
        console.log("✅ Vault secret cleaned up");
      } catch (error) {
        console.error("⚠️ Failed to clean up Vault secret:", error);
      }
    }
    if (prisma) {
...
describe("Vault Integration Test Status", () => {
  it("should show Vault test status", () => {
    if (SHOULD_RUN) {
      ...
      console.log(
        "⏭️  Vault tests SKIPPED - add DIRECT_URL to .env.test.local",
      );
      console.log(
        "     DIRECT_URL should be: postgresql://...@...supabase.com:5432/postgres (port 5432, not 6543)",
      );
    }
